...
- **`sf`**: trỏ về đúng 1 `id` trong mảng `sfs` của CHÍNH scene đó — đây là ảnh neo duy nhất cho toàn bộ video 10s (cả 2 shot con). Không video nào được neo vào 2 ảnh.
...
**Tỉ lệ khung của cả dự án là DỌC 9:16.** Mọi ảnh sinh ra trong `sf-board.json` — thẻ nhân vật (`REF_*_PORTRAIT`, `REF_*_FULL`), thẻ đạo cụ (`REF_PROP_*`), Thẻ Địa Điểm (`REF_BG_*`), Master SF và SF thường — đều là khung dọc 9:16. Không có ngoại lệ, không có ảnh ngang trong dự án Short.

**Dòng kết thúc bắt buộc: Prompt của MỌI thẻ ảnh — thẻ nhân vật (`REF_*_PORTRAIT`, `REF_*_FULL`), thẻ đạo cụ (`REF_PROP_*`), Thẻ Địa Điểm (`REF_BG_*`) và mọi Start Frame (`SF-*` trong `sfs[]`, gồm cả Master SF và SF thường)** bắt buộc kết thúc bằng dòng `KHUNG DỌC 9:16` ở dòng cuối cùng của prompt.
...
- [ ] Mọi thẻ ảnh (`REF_*` và `SF-*`) kết thúc bằng đúng dòng `KHUNG DỌC 9:16`.
- [ ] Không thẻ ảnh nào trong board còn là khung ngang — mọi thẻ REF / Master SF / SF đều 9:16.
