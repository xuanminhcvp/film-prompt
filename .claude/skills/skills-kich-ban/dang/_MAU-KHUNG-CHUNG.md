# MẪU KHUNG CHUNG — chép file này, đừng sửa file này

> **Cách dùng:** `cp _MAU-KHUNG-CHUNG.md <TÊN-KHUNG>.md` rồi điền. Quy trình đầy đủ ở
...
> 4. **Mỗi ô phải mang một nhãn** — `[BẮT BUỘC]` (yêu cầu có thật của kênh, cứng) · `[MỤC TIÊU]` (việc phải làm được, tự do về cách) · `[THAM KHẢO]` (kho hay làm thế, không phải luật). Số đo được từ một bộ mẫu hữu hạn mặc định là `[THAM KHẢO]`; muốn nâng lên `[MỤC TIÊU]` thì phải trả lời được *bỏ nó đi thì truyện hỏng ở chỗ nào*.
...
## 4. Dấu nhận dạng của dạng này · [THAM KHẢO]
...
| Cấm |
|---|
| <việc dạng khác làm được mà dạng này không> |
