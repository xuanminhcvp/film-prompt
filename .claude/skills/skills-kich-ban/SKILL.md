...
description: Viết trọn một kịch bản kể chuyện (story) từ ý tưởng đến file — chọn dạng bài (random nếu user không chỉ định), title, hook, rồi toàn bộ phần sau hook, tự kiểm chất lượng ở mỗi bước và chạy thẳng không hỏi lại — xuất ra `KICH-BAN.md` trong `<TEN-DU-AN>.project/`. Hỗ trợ tám dạng bài (REVEAL · CHALLENGE · KINDNESS · FIGHT · WARNING · TEST · REUNION · INVESTIGATION) dùng chung khung MUSE trong `dang/`. Dùng skill này mỗi khi user nói "viết kịch bản mới", "nghĩ title", "viết hook cho phim này", "viết tiếp phần sau hook", "làm story về ...", kể cả khi user chỉ ném một ý tưởng một dòng. KHÔNG dùng cho khâu dựng hình (SF, prompt video, nhạc) — đó là skills-film / skills-hook.
---
...
| **Dạng bài** | `dang/muse/<TÊN>.md` | *Cái gì khởi động* · *cú lật là gì* · trục biến thiên | Đổi 6 thứ |

**Hiện có tám dạng bài**, tất cả dùng chung khung `MUSE`: `REVEAL` · `CHALLENGE` · `KINDNESS` · `FIGHT` · `WARNING` · `TEST` · `REUNION` · `INVESTIGATION`. Bảng đầy đủ ở [dang/MUSE.md](dang/MUSE.md) §8.

Lõi **khai báo tên tham số** (`HOOK.do-dai`, `THAN.khung-muc`, …) và nói "tra ở khung chung".
Khung chung **điền giá trị** cho đúng những tham số ấy. Dạng bài **chỉ ghi đè những gì `MUSE.md` §8 liệt kê** và cấm chép lại phần còn lại.

Mỗi lượt làm việc chỉ nạp: **lõi cần cho bước đang làm + `dang/MUSE.md` + đúng MỘT dạng bài.**
...
|---|---|---|
| **[SẢN XUẤT]** | Yêu cầu có thật của kênh/khách (độ dài video, CTA, trần title) | Không |
| **[NHIỆM VỤ]** | Việc đoạn văn phải làm được (tạo tò mò · trả đúng nợ · chịu thiệt trước cú lật) | Không — nhưng **tự do hoàn toàn về cách làm** |
| **[QUAN SÁT]** | Kho tham chiếu hay làm thế (thứ tự các bước · tỉ lệ câu ngắn · motif · công thức title) | Có, và **nên lệch khi truyện đòi** |

Đối xử với một định mức [QUAN SÁT] như thể nó là [SẢN XUẤT] chính là cách biến mọi kịch bản mới thành bản sao thống kê của kho cũ. Luật đầy đủ và sáu lỗi cần tránh: [8-chong-rap-khuon.md](references/8-chong-rap-khuon.md) — **file đó có quyền phủ quyết mọi luật khác trong skill này.**
...
|---|---|---|---|
| **1** Chọn dạng bài | User có ý tưởng → chọn dạng khớp *khởi động* + *cú lật*. Không có ý tưởng → **random**, trừ dạng của hai bài gần nhất | [1-chon-dang](references/1-chon-dang.md) | Ghi ra `DẠNG BÀI` · `SỰ THẬT LÕI` · `LỆCH CÓ CHỦ Ý`. Báo cho user biết đã chọn dạng nào, rồi **đi tiếp luôn** |
...
| **3** Hook | 115–175 từ, tám nhiệm vụ | [3-hook](references/3-hook.md) · [5-nhan-vat](references/5-nhan-vat-va-chi-tiet.md) · [6-giong-van](references/6-giong-van.md) | `3-hook.md` §6 + `MUSE.md` §7 phần hook + checklist dạng bài. Đếm từ thật. Tách riêng **câu lời hứa** để bước 6 đối chiếu |
| **4a** Thân — dựng người | Giai đoạn 1–3 | [4-than-truyen](references/4-than-truyen.md) · [5-nhan-vat](references/5-nhan-vat-va-chi-tiet.md) · [6-giong-van](references/6-giong-van.md) | Đủ việc của ba giai đoạn theo §3 dạng bài · nhịp câu đạt · đã gài xong mọi thứ giai đoạn 6 sẽ dùng |
| **4b** Thân — sự việc | Giai đoạn 4–5 | như trên | Mất mát có thật và **đúng kiểu của dạng bài** · chưa lộ cú lật |
...
| **5** Rà chống rập khuôn | Ba câu hỏi + checklist | [8-chong-rap-khuon](references/8-chong-rap-khuon.md) | `8-chong-rap-khuon.md` §4. Không nêu được chỗ đã lệch khỏi mẫu hình → **quay lại sửa**, không phải ghi chú rồi cho qua |
...
**Ghi file từ bước 3, không đợi bước 6.** Hook qua cửa là ghi ngay vào `KICH-BAN.md`; mỗi lượt 4a/4b/4c ghi nối tiếp. Bước 6 chỉ chốt sổ.
...
Hai dạng vừa dùng bị loại khỏi lượt bốc này. Tỉ trọng kho ở `MUSE.md` §8 **không phải chỉ tiêu** — `INVESTIGATION` chiếm 2% kho nhưng là dạng khó lặp lại nhất, đừng né nó.
...
| **Ba loại ràng buộc · sáu lỗi rập khuôn · cấm chép ví dụ · rà trước khi giao** | `references/8-chong-rap-khuon.md` |
| **Mọi giá trị định mức** (độ dài, nhịp câu, CTA, trần title) · tám nhiệm vụ hook · bảy nhiệm vụ thân | `dang/MUSE.md` |
| **Cái gì khởi động · cú lật là gì · công thức vế trước của title · cấm riêng** | `dang/muse/<DẠNG-BÀI>.md` |

**Luật phân tầng, dùng khi không biết viết luật mới vào đâu:**

> Đúng bất kể viết gì → `references/`.
> Đúng với cả tám dạng bài, đổi thì tám cái cùng đổi → `dang/MUSE.md`.
> Chỉ đúng với một kiểu chuyện → `dang/muse/<DẠNG-BÀI>.md`.
...
- **Luật mới mặc định là [QUAN SÁT].** Muốn nâng lên [NHIỆM VỤ] phải trả lời được: *bỏ nó đi thì truyện hỏng ở chỗ nào?* Không trả lời được thì nó là thói quen, không phải luật.
- **Mỗi "cấm" phải kèm lý do hỏng gì.** Cấm không nêu được hậu quả là sở thích cá nhân đang đội lốt luật.
- **Thêm tham số mới** = sửa bảng §2 của `1-chon-dang.md` + sửa file lõi khai báo nó + điền vào **mọi** hồ sơ dạng đang có + sửa `dang/_MAU-DANG.md`. Không làm đủ ba việc thì đừng thêm.
...
| Bước 3 — hook | `references/3-hook.md` §6 · `dang/MUSE.md` §7 · checklist cuối dạng bài |
| Bước 4a–4c — thân | `references/4-than-truyen.md` §7 · `dang/MUSE.md` §7 · checklist cuối dạng bài |
...
| **Bước 5 — rà chống rập khuôn** | `references/8-chong-rap-khuon.md` §4 |
| Bước 6 — ghi file & bàn giao | `references/7-file-va-ban-giao.md` §6 |

Cả ba checklist **đều phải chạy**: lõi kiểm form, khung chung kiểm số, dạng bài kiểm cú lật. Rồi `8-chong-rap-khuon.md` kiểm xem bài có phải bản sao của kho không.
...
- **Chốt một chỗ lệch có chủ ý** ngay ở bước 1 — chỗ định làm khác kho tham chiếu, và vì sao.
