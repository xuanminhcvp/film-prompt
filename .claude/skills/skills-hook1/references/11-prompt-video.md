...
- **Mặc định chuẩn 10 giây (`dur: 10`):** mọi video mặc định sinh ở mốc 10s, gồm 2 shot con nối bằng 1 cú HARD CUT dứt khoát (`10-SECOND VIDEO. EXACTLY TWO SHOTS. EXACTLY ONE HARD CUT.`).
- **Ngoại lệ 6 giây (`dur: 6`):** chỉ cho cặp clip tách từ chuỗi thoại dồn (`7-bang-shot.md` §5.2b) — form §3.1b.
- **Prompt không nén:** prompt video mặc định viết bằng tiếng Anh, đầy đủ, không nén. Bỏ tư duy rút gọn để ép vừa ký tự cũ. Trần ký tự của prompt video phim/hook: 2.000 – 2.800 ký tự; clip 6s chỉ áp trần 2.800, không bắt buộc sàn 2.000.
...
### 3.1b Form Video 6 giây (2 Shot, 1 Hard Cut) — chỉ cho chuỗi thoại dồn
Giữ nguyên toàn bộ form §3.1, chỉ khác:
- Dòng mở: `6-SECOND VIDEO. EXACTLY TWO SHOTS. EXACTLY ONE HARD CUT.`
- Mọi mốc giây (TIMING, tiêu đề `SHOT N`, `CUT 1`) tính trên khung **0:00–0:06**; tính bằng 3.0 từ/giây rồi đặt cú cắt ngay tại lượt đổi người nói hoặc chỗ bị cắt lời — không mặc định chia đôi 3s/3s.
- Không viết khoảng lặng dài để lấp cho đủ giờ: clip 6s được sinh ra chính để giữ nhịp dồn.

### 3.2 Form Video 15 giây (3 Shot, 2 Hard Cut) (KHÔNG DÙNG CHO HOOK)
```text
...
- Con số 3.0 từ/giây chỉ là trần tối đa chống nhồi thoại. Trần cứng theo `7-bang-shot.md` §5.2 (28 từ cho clip 10s, 16 từ cho clip 6s).
...
⛔ **Chọn hành vi theo HOÀN CẢNH của đúng khoảnh khắc đó — không theo nghề nghiệp.** Danh sách trên
là kho chung, không phải mặc định. Trước mỗi cử chỉ, hỏi đúng hai câu:
1. *Ngay giây này nhân vật đang chịu điều gì, và họ đang giữ mình vì ai?* (bị sỉ nhục trước mặt con,
   con đang đứng trước toà, sắp mất việc, bị cả phòng nhìn…)
2. *Một người thật trong đúng hoàn cảnh ấy, cơ thể phản ứng ra sao?* Cử chỉ phải là **phản ứng với điều
   vừa xảy ra** — hướng về người/lời vừa làm họ đau, hoặc về người họ đang bảo vệ.

**Phép thử loại bỏ:** nếu chỉ một người **không quan tâm** tới chuyện đang diễn ra mới làm cử chỉ đó
(liếc đồng hồ, xem điện thoại, chỉnh tay áo, làm một thao tác nghề quen tay…) thì bỏ — dù nó hợp nghề
đến đâu. Lỗi có thật: một người mẹ làm điều phối viên cấp cứu được cho "liếc đồng hồ như ghi giờ cuộc
gọi" đúng lúc thẩm phán gọi bà là rác trước mặt đứa con đang bị xử — người xem đọc ra là bà thờ ơ.

Nghề nghiệp chỉ là lớp tô thêm, được dùng khi nó **trùng** với phản ứng cảm xúc, không bao giờ thay
nó. VD kỳ thủ bị giật mất quân cờ: **mắt rơi xuống đúng ô vừa trống** — đúng vì đó là chỗ anh ta vừa
bị tước mất (phản ứng với nỗi đau), nghề chỉ làm phản ứng ấy cụ thể hơn. Rồi để một từ trong câu thoại
kéo mắt anh ta ngẩng lên: *"his eyes come back up on the word 'mother'"*.
...
2. **Mặt rõ mọc từ vùng mờ/quay lưng**: SF là OTS hoặc góc chỉ thấy vai/gáy mờ của một nhân vật (không thấy mặt) — Shot 2 tuyệt đối không được "cắt sang khuôn mặt rõ nét" của đúng người đó. Nếu người đó có thoại trong clip thì **ảnh SF đã chọn sai góc** — người nói phải rõ mặt (`7-bang-shot.md` §5.3): sửa lại SF neo sang góc thấy mặt họ, không để họ nói bằng gáy. Chỉ câu chen ngang thuộc ngoại lệ ở `7-bang-shot.md` §5.3 mới được nói với mặt quay đi.
...
**Đếm được:**
- [ ] Prompt video **trong khoảng 2.000 – 2.800 ký tự** (§1); clip 6s chỉ cần **≤2.800**.
- [ ] Clip `dur: 6` dùng dòng mở `6-SECOND VIDEO…` và mọi mốc giây nằm trong **0:00–0:06** (§3.1b).
...
- [ ] Mỗi hành vi kìm nén của chính diện là phản ứng với đúng hoàn cảnh của khoảnh khắc đó (lời vừa bị nói, người họ đang bảo vệ), không lấy mặc định từ danh sách; không có cử chỉ nào mà chỉ người thờ ơ mới làm (liếc đồng hồ, xem điện thoại, thao tác nghề quen tay) (§4.5b).
...
- [ ] Người thoại luôn RÕ MẶT trong shot con có câu của họ (ngoại lệ chen ngang: `7-bang-shot.md` §5.3; qua điện thoại/loa: §5.4).
