...
- **Hook có thể được nối dài nhiều lượt**: user có quyền dán thêm đoạn tiếp nối ("làm thêm cho đoạn hook đến hết đây") sau khi Hook ban đầu đã dựng xong. Cộng dồn số từ cũ + đoạn mới rồi áp lại toàn bộ §2 từ đầu (kể cả khi đã vượt trần — xử lý ở §2.3).

## 2. Quét kịch bản để dựng Hook
- **Luật 2 phút Hook**: Bảng shot sản xuất của Hook chỉ kéo dài **ĐÚNG 2 PHÚT (120 giây)**.

- **Xử lý khi user gửi kịch bản dài**: Dù user có cung cấp cả một kịch bản phim dài đầy đủ, kịch bản đó **chỉ dùng để lấy ngữ cảnh câu chuyện, bối cảnh địa điểm và dựng REF nhân vật chuẩn**. Phần phân rã bảng shot trong `sf-board.json` chỉ tập trung đúng đoạn Hook.
- **Đối chiếu mọi Heading `##`**: bắt buộc quét các heading `##` nằm trong phân đoạn Hook được chọn (như `## HOOK` hay `## SCENE 1`), tuyệt đối không bỏ lọt.

### 2.1 Tính thời lượng từ số chữ
- **Đếm số từ đoạn Hook TRƯỚC khi dựng bất cứ thứ gì**.
- Công thức: **Số shot = ceil(số từ ÷ 26)**. Tổng thời lượng = **số shot × 10s**.
- Nếu có chuỗi thoại dồn phải tách thành 2 clip 6s (`7-bang-shot.md` §5.2b): mỗi lần tách +1 shot, +2s. Báo user con số **sau khi tách**.
...
Cộng dồn Hook cũ + đoạn nối thêm có thể vượt 12 shot/120s. Đây là luật **tuyệt đối**, không tự ý bỏ qua:
- **Bắt buộc báo user** con số vượt (bao nhiêu từ → bao nhiêu shot → vượt bao nhiêu giây) **trước khi dựng thêm bất cứ gì**.
- **Hỏi user chọn hướng xử lý** (không tự quyết): (a) vẫn dựng đủ, chấp nhận vượt trần, hoặc (b) nén/gộp bớt để vừa đúng 120s, hoặc (c) tách đoạn mới thành scene riêng (không tính vào Hook). Nếu user xác nhận muốn vượt trần, tôn trọng quyết định đó và dựng đủ.
...
## 5. Kiểm tra
- [ ] Số shot khớp công thức ceil(số từ ÷ 26), cộng 1 cho mỗi lần tách chuỗi thoại dồn (`7-bang-shot.md` §5.2b); đã báo user tổng thời lượng trước khi dựng. Kịch bản dài (nếu có) chỉ dùng để lấy ngữ cảnh & nhân vật chuẩn.
