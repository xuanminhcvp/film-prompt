...
### 2.6 Cấp SF — Thumbnail (cuối mảng `sfs` của scene `REF`)
3 SF thumbnail (`SF-THUMB-01/02/03`) nằm **cuối** mảng `sfs[]` của scene `REF`, sau mọi thẻ nhân vật / đạo cụ / địa điểm. Không tạo scene riêng cho thumbnail. Nội dung prompt và `notes`: `12-thumbnail.md`.
```json
{
  "id": "SF-THUMB-01",
  "label": "★ THUMBNAIL #1 (Template A)",
  "prompt": "[Photorealistic, Highly detailed, Professional Photography, 16:9] ...\nKHÔNG chữ, KHÔNG watermark, KHÔNG logo, KHÔNG vòng tròn đỏ, KHÔNG mũi tên.\nKHUNG NGANG 16:9",
  "status": "draft",
  "notes": "Nguồn: Template Diner hàng 1\nMenu tinh chỉnh:\n1. ...",
  "refs": {
    "chars": ["REF_TEN_NHAN_VAT_WORK_FULL"],
    "bg": "REF_BG_BOI_CANH"
  },
  "picked": ""
}
...
| SF thumbnail | `SF-THUMB-0<n>` (n = 1…3) | `SF-THUMB-01` · `SF-THUMB-03` |
...
- **SF thumbnail** không phải SF thường: `refs.bg` trỏ thẳng Thẻ Địa Điểm (`REF_BG_*`), `refs.chars` chỉ chứa thẻ `_FULL`. Không shot nào trỏ tới nó — đó là bình thường, không tính là SF mồ côi.
...
| SF thumbnail | THUMB-01/02: xấp xỉ mẫu template gốc, `lệch ≤ 10%` (dùng nguyên văn mẫu, chỉ đổi người + bối cảnh) · THUMB-03: `≤ 3.500 ký tự` (≈ trung vị kho mẫu) |
...
- [ ] Prompt thẻ địa điểm / Master SF **≤1.400 ký tự**; SF thường **<1.000 ký tự**; SF-THUMB-03 **≤3.500 ký tự**, SF-THUMB-01/02 lệch **≤10%** so với độ dài mẫu gốc.
- [ ] Đúng 3 `SF-THUMB-0<n>` nằm cuối `sfs[]` của scene `REF` (§2.6).
...
- [ ] Không SF nào mồ côi (không shot nào trỏ tới) — trừ `SF-THUMB-*` (§5) — và không hai shot nào dùng chung một giá trị `sf` (§5.2).
